...
import { editBookThunk } from "redux/asyncThunks/bookThunks";
import { fetchAuthorThunk } from "redux/asyncThunks/authorThunks";
import { bookSchema, BookFormData } from "validation";
import { Book } from "types";

const EditBookModal = ({_id, isbn, title, description, publisher, publishedYear, category}: Book) => {
  const dispatch = useAppDispatch();
  const author = useAppSelector((state) => state.authors.items);
  ...
  const onSubmit = handleSubmit((data) => {
    //create new type for Book update
    dispatch(editBookThunk(data.body as Book));
    handleModal();
  });
